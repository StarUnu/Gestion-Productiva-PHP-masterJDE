...
foreach( ($unidad==-1 ? $this->model->Buscar($startFrom,$search) : $this->model->BuscarByUnidadId($startFrom, $search, $unidad) )  as $r):
    ...
    $output .='<tr>
              ...
              <td>'.$this->model->ObtenerNombreUnidadProductiva($r->Unidad_Id)->Nombre.'</td>
                ...
                <div class="btn-group">
                    <a class="btn btn-xs btn-warning buttonCrud"   href="'.BASE_URL.'InventariodeSeresVivos/Crud/'.$r->Id.'"'.'><span class="glyphicon glyphicon-pencil"></span></a>';
                
              
if ( $r->Unidad_Id==$_SESSION["Unidad_Id"] ) 
  {$output.= '<a onclick="javascript:return confirm('."'¿Seguro de eliminar este registro?'".');"'.'class="btn btn-xs btn-danger buttonCrud" href="<?php echo BASE_URL; ?>InventariodeSeresVivos/Eliminar/<?php echo $r->Id; ?>"><span class="glyphicon glyphicon-trash"></span> </a>
    </div>
    </td>
    </tr>';
}
...
$output.=   '</tbody>
            ...
            <nav><ul class="pagination">';
                if(!empty($totalPages)):for($i=1; $i<=$totalPages; $i++):  
                  if($i == 1):
                    $output.= '<li class="active"  id="'.$i.'"><a href="'.BASE_URL.'InventariodeSeresVivos/Paginacion/Pagina/'.$i.'/Busqueda/'.$search.'">'.$i.'</a></li>'; 
                  else:
                    $output.= '<li id="'.$i.'"><a href="'.BASE_URL.'InventariodeSeresVivos/Paginacion/Pagina/'.$i.'/Busqueda/'.$search.'">'.$i.'</a></li>';
                  endif;
                endfor;endif;
            $output.= '</ul></nav>';
